...
- `examples/monitoring/kube-state-metrics-crd-config.yaml` provides a
  ...
  `PromotionTrigger`, and `PluginRegistration` status fields.
...
["promotionruns", "promotiontriggers", "pluginregistrations"]`, and `verbs:
...
| PromotionTrigger is blocked by cooldown, max-active, signature, or source errors. | `kapro_promotiontrigger_status_condition{type,reason}` until a controller-owned blocked counter exists. |
