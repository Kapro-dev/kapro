// Package gate defines the public Kapro programmable gate SDK.
//
...
//
// External implementations can implement Predicate and wire in at startup via
// the gate registry used by the promotionrun controller.
//
// # The CRI analogy
//
// Kapro is to gates what Kubernetes is to containers:
//   - Kapro manages gate lifecycle (when, timeout, retry, failure policy)
//   - Predicate.Evaluate() is the KGI contract — analogous to CRI's RunPodSandbox
...
// # Stability
//
// KGI v1alpha1 is stable. Predicate is the canonical authoring contract; Gate
// remains an alias for users who prefer Kapro's domain vocabulary.
package gate
...
type Request struct {
	...
	// It may be nil for SDK tests, plugin probes, and purely programmable
	// predicates. Most built-in predicates require it.
	Context *Context
...
// Predicate is KGI v1alpha1: the Kapro Gate Interface.
//
...
// PredicateFunc adapts a plain function into a Predicate. This is the
// Knative-functions authoring shape narrowed to "may this promotion proceed?"
type PredicateFunc func(ctx context.Context, req Request) (Result, error)

// Evaluate implements Predicate.
func (f PredicateFunc) Evaluate(ctx context.Context, req Request) (Result, error) {
	return f(ctx, req)
}

// Gate is a compatibility alias for Predicate.
type Gate = Predicate

// Func is a compatibility alias for PredicateFunc.
...
const (
	...
	Passed       = PhasePassed
	Failed       = PhaseFailed
	Inconclusive = PhaseInconclusive
)
...
func Recover(next Predicate) Predicate {
	return PredicateFunc(func(ctx context.Context, req Request) (result Result, err error) {
		defer func() {
			if recovered := recover(); recovered != nil {
				result = MakeFailed("PanicRecovered", "predicate panic: %v", recovered)
				err = nil
			}
		}()
