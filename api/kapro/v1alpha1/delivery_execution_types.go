// Plan template, delivery execution, and per-target rollout types plus their
// shared stage primitives. Plan is the only CRD root in this public package;
// PromotionRun and Target roots live in runtime.kapro.io and alias the shared
// spec/status structs from this file.
package v1alpha1
...
// Validation is enforced by the admission webhook. Execution state lives in
// PromotionRun. Deleting a Plan does not cascade to runtime records; a pending
// or in-flight PromotionRun that still references the deleted Plan fails with
// PromotionPlanNotFound.
type Plan struct {
...
type PromotionRunSpec struct {
	...
	// Versions maps promotion unit name to the substrate-native revision to
...
type TargetExecutionState struct {
	...
	// SubstrateObjects records the substrate-native objects this target expects to
	// converge, for example Argo CD Applications selected by a label selector.
	// It is status evidence only; substrate adapters own the actual resources.
	// +optional
	SubstrateObjects []SubstrateObjectStatus `json:"substrateObjects,omitempty"`
...
// SubstrateObjectStatus reports the health of one substrate-native object expected
// to converge for a Target.
type SubstrateObjectStatus struct {
	// APIVersion is the substrate object's API version.
	// +optional
	APIVersion string `json:"apiVersion,omitempty"`
	// Kind is the substrate object's kind.
	// +optional
	Kind string `json:"kind,omitempty"`
	// Namespace is the substrate object's namespace.
	// +optional
	Namespace string `json:"namespace,omitempty"`
	// Name is the substrate object's name.
	// +optional
	...
	// CurrentVersion is the revision currently reported by the substrate object.
	// +optional
	CurrentVersion string `json:"currentVersion,omitempty"`
	// SyncStatus is the substrate sync status when available.
	// +optional
	SyncStatus string `json:"syncStatus,omitempty"`
	// HealthStatus is the substrate health status when available.
	// +optional
