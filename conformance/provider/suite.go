...
func RunSuite(t *testing.T, c kci.Connector) {
	...
	t.Run("KCI/NilEnvironment", func(t *testing.T) { testNilEnvironment(t, c) })
...
func testContextCancellation(t *testing.T, c kci.Connector) {
	...
	go func() {
		defer close(done)
		c.Connect(ctx, minimalEnv()) //nolint:errcheck
	}()
...
// testNilEnvironment verifies Connect/IsReachable handle nil without panicking.
func testNilEnvironment(t *testing.T, c kci.Connector) {
	t.Helper()
	defer func() {
		if r := recover(); r != nil {
			t.Errorf("Connector panicked on nil Environment: %v", r)
		}
	}()
...
func testIsReachableReturnsBool(t *testing.T, c kci.Connector) {
	...
	defer cancel()
	_, _ = c.IsReachable(ctx, minimalEnv())
}
...
func testConnectReturnsCfgOrError(t *testing.T, c kci.Connector) {
	...
	cfg, err := c.Connect(ctx, minimalEnv())
	if cfg == nil && err == nil {
...
func testConcurrentSafe(t *testing.T, c kci.Connector) {
	...
	for i := 0; i < goroutines; i++ {
		go func() {
			ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
			defer cancel()
			c.IsReachable(ctx, minimalEnv()) //nolint:errcheck
			done <- struct{}{}
		}()
	}
	for i := 0; i < goroutines; i++ {
...
func minimalEnv() *kaprov1alpha1.Environment {
	e := &kaprov1alpha1.Environment{
		ObjectMeta: metav1.ObjectMeta{Name: "conformance-env"},
	}
	e.Spec.Provider = &kaprov1alpha1.ProviderSpec{}
	return e
}
