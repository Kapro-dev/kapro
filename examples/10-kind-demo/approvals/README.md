...
Approval manifests used by the Kind demo.

```text
PromotionRun pause -> Approval -> stage resumes
```

Apply when the demo is waiting for production approval.
...
```bash
kubectl delete -f examples/10-kind-demo/approvals --ignore-not-found
```
