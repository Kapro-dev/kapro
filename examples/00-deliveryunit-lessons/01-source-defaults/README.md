...
```bash
examples/00-deliveryunit-lessons/01-source-defaults/run.sh
```

After you have a Kubernetes cluster and the required controllers installed, apply the manifests through the wrapper:

```bash
examples/00-deliveryunit-lessons/01-source-defaults/run.sh apply
```
...
```bash
kubectl delete -f examples/00-deliveryunit-lessons/01-source-defaults --ignore-not-found
```
