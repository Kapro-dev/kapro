...
```bash
go run ./examples/05-plugins/00-argocd-actuator --listen :9090 --namespace argocd
```
...
```bash
docker build -f examples/05-plugins/00-argocd-actuator/Dockerfile \
...
`examples/05-plugins/00-argocd-actuator/manifests/deployment.yaml`. It creates:
...
`examples/05-plugins/00-argocd-actuator/registration.yaml`:
...
```bash
go test ./examples/05-plugins/00-argocd-actuator
```
