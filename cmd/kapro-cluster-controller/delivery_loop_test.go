...
func newDeliveryFC(name string, desired map[string]string, suspend bool, backend string) *kaprov1alpha2.Cluster {
	fc := &kaprov1alpha2.Cluster{}
	fc.Name = name
	fc.Spec.DesiredVersions = desired
	fc.Spec.Suspend = suspend
	fc.Spec.Delivery = kaprov1alpha2.DeliverySpec{
...
func newDeliveryBP(name string, driver kaprov1alpha2.BackendDriver) *kaprov1alpha2.Backend {
	bp := &kaprov1alpha2.Backend{}
	bp.Name = name
	bp.Spec.Driver = driver
	bp.Spec.Runtime = kaprov1alpha2.BackendRuntimeSpoke
	return bp
}

func deliveryHub(t *testing.T, fc *kaprov1alpha2.Cluster, bp *kaprov1alpha2.Backend) client.Client {
	t.Helper()
	scheme := newStatusScheme(t)
	b := fake.NewClientBuilder().WithScheme(scheme).WithStatusSubresource(&kaprov1alpha2.Cluster{})
	if fc != nil {
		b = b.WithObjects(fc)
	}
	if bp != nil {
...
func TestDeliveryLoop_ConvergedAdvancesCurrentVersions(t *testing.T) {
	...
	provider := &scriptedProvider{
		...
		results: map[string]spokeprovider.ReconcileResult{
			"api": {Phase: kaprov1alpha2.DeliveryPhaseConverged, ObservedDigest: "sha256:aaa", AppliedObjects: 5, Format: "raw-yaml"},
			"web": {Phase: kaprov1alpha2.DeliveryPhaseConverged, ObservedDigest: "sha256:bbb", AppliedObjects: 3, Format: "helm"},
		},
	}
	reg := spokeprovider.NewRegistry()
	if err := reg.Register(kaprov1alpha2.BackendDriverOCI, provider); err != nil {
	...
	if got.Status.Delivery["api"].Phase != kaprov1alpha2.DeliveryPhaseConverged {
...
func TestDeliveryLoop_FailedDoesNotAdvanceCurrentVersions(t *testing.T) {
	...
	if entry.Phase != kaprov1alpha2.DeliveryPhaseFailed {
...
func TestDeliveryLoop_SuspendedWritesSkipped(t *testing.T) {
	...
	provider := &scriptedProvider{driver: kaprov1alpha2.BackendDriverOCI}
	reg := spokeprovider.NewRegistry()
	_ = reg.Register(kaprov1alpha2.BackendDriverOCI, provider)
	l := &deliveryLoop{
		Hub:         newHubClientFromStatic(hub),
	...
	for _, app := range []string{"api", "web"} {
		if got.Status.Delivery[app].Phase != kaprov1alpha2.DeliveryPhaseSkipped {
...
func TestDeliveryLoop_UnknownDriverFailsLoudly(t *testing.T) {
	...
	bp := newDeliveryBP("external-1", kaprov1alpha2.BackendDriverExternal)
	hub := deliveryHub(t, fc, bp)

	// Registry has only oci, not external. Resolve will fail.
	reg := spokeprovider.NewRegistry()
	_ = reg.Register(kaprov1alpha2.BackendDriverOCI, &scriptedProvider{driver: kaprov1alpha2.BackendDriverOCI})
	l := &deliveryLoop{
		Hub:         newHubClientFromStatic(hub),
		ClusterName: "c1",
		Registry:    reg,
...
func TestDeliveryLoop_HubRuntimeProfileSkipsSpokeDelivery(t *testing.T) {
	...
	bp := newDeliveryBP("flux-hub", kaprov1alpha2.BackendDriverFlux)
	bp.Spec.Runtime = kaprov1alpha2.BackendRuntimeHub
	hub := deliveryHub(t, fc, bp)
	...
	entry := got.Status.Delivery["api"]
	if entry.Phase != kaprov1alpha2.DeliveryPhaseSkipped {
...
func TestDeliveryLoop_BothRuntimeProfileRunsOnSpoke(t *testing.T) {
	...
	bp := newDeliveryBP("oci-both", kaprov1alpha2.BackendDriverOCI)
	bp.Spec.Runtime = kaprov1alpha2.BackendRuntimeBoth
	hub := deliveryHub(t, fc, bp)
...
func TestDeliveryLoop_BackfillsLastAttemptedAtWhenProviderOmitsIt(t *testing.T) {
	...
	provider := &scriptedProvider{
		driver: kaprov1alpha2.BackendDriverOCI,
		results: map[string]spokeprovider.ReconcileResult{
			"api": {Phase: kaprov1alpha2.DeliveryPhaseConverged}, // no LastAttemptedAt
		},
	}
	reg := spokeprovider.NewRegistry()
	_ = reg.Register(kaprov1alpha2.BackendDriverOCI, provider)
