...
`v0.1.0` is the first public preview release for controlled adopters. It is
not a GA stability promise. Update `CHANGELOG.md` before tagging and use
...
For tag `v0.1.0`, the chart package version does not include the leading `v`:
publish `kapro-operator-0.1.0.tgz` and
`kapro-cluster-controller-0.1.0.tgz` as release assets.
...
defect is discovered. UX improvements after `v0.1.0` should preserve the
