...
VERSION=v0.6.2 scripts/ci-release-smoke.sh
...
`v0.6.2` is the current public preview release candidate for controlled
...
For tag `v0.6.2`, the chart package version does not include the leading `v`:
publish `kapro-operator-0.6.2.tgz` and
`kapro-cluster-controller-0.6.2.tgz` as release assets.
...
defect is discovered. UX improvements after `v0.6.2` should preserve the
