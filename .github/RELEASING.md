...
VERSION=v0.1.2 scripts/ci-release-smoke.sh
...
`v0.1.2` is the current public preview release candidate for controlled
...
- all Kubernetes APIs are `kapro.io/v1alpha2`;
...
For tag `v0.1.2`, the chart package version does not include the leading `v`:
publish `kapro-operator-0.1.2.tgz` and
`kapro-cluster-controller-0.1.2.tgz` as release assets.
...
both Helm charts, uploads checksums, and creates a GitHub release marked as a
pre-release for the `v0.x` public preview line.
...
defect is discovered. UX improvements after `v0.1.2` should preserve the
`Fleet`, `Source`, `Plan`, `Promotion`, `PromotionRun`, and `Target` contract.
