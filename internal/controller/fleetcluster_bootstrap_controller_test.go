...
func TestComputeExpiresAt_HonoursTTL(t *testing.T) {
	...
	fc := &kaprov1alpha1.FleetCluster{
		ObjectMeta: metav1.ObjectMeta{Name: "de-prod-01", CreationTimestamp: created},
		Spec: kaprov1alpha1.FleetClusterSpec{
			Bootstrap: &kaprov1alpha1.FleetClusterBootstrapSpec{TTL: "6h"},
		},
	}
	r, c := newBootstrapReconciler(t, fc)
...
func TestComputeExpiresAt_CapsAtMax(t *testing.T) {
	...
	fc := &kaprov1alpha1.FleetCluster{
		ObjectMeta: metav1.ObjectMeta{Name: "de-prod-01", CreationTimestamp: created},
		Spec: kaprov1alpha1.FleetClusterSpec{
			Bootstrap: &kaprov1alpha1.FleetClusterBootstrapSpec{TTL: "720h"}, // 30 days, way over max
		},
	}
	r, c := newBootstrapReconciler(t, fc)
...
func TestComputeExpiresAt_RejectsInvalidTTL(t *testing.T) {
	fc := &kaprov1alpha1.FleetCluster{
		...
		Spec: kaprov1alpha1.FleetClusterSpec{
			Bootstrap: &kaprov1alpha1.FleetClusterBootstrapSpec{TTL: "garbage"},
		},
	}
...
func TestComputeExpiresAt_RejectsZeroTTL(t *testing.T) {
	fc := &kaprov1alpha1.FleetCluster{
		ObjectMeta: metav1.ObjectMeta{Name: "de-prod-01"},
		Spec: kaprov1alpha1.FleetClusterSpec{
			Bootstrap: &kaprov1alpha1.FleetClusterBootstrapSpec{TTL: "0s"},
		},
	}
	r, _ := newBootstrapReconciler(t, fc)
...
func TestComputeExpiresAt_NoopWhenAlreadySet(t *testing.T) {
	...
	fc := &kaprov1alpha1.FleetCluster{
		ObjectMeta: metav1.ObjectMeta{Name: "de-prod-01"},
		Spec: kaprov1alpha1.FleetClusterSpec{
			Bootstrap: &kaprov1alpha1.FleetClusterBootstrapSpec{ExpiresAt: &already},
		},
	}
	r, _ := newBootstrapReconciler(t, fc)
...
func TestExpired(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			fc: &kaprov1alpha1.FleetCluster{
				Spec: kaprov1alpha1.FleetClusterSpec{Bootstrap: &kaprov1alpha1.FleetClusterBootstrapSpec{ExpiresAt: &past}},
			},
			want: true,
		},
		{
			...
			fc: &kaprov1alpha1.FleetCluster{
				Spec: kaprov1alpha1.FleetClusterSpec{Bootstrap: &kaprov1alpha1.FleetClusterBootstrapSpec{ExpiresAt: &future}},
			},
			want: false,
		},
		{
...
func TestHandleSuspended_SetsReconcilingFalseAndClearsStalled(t *testing.T) {
	fc := &kaprov1alpha1.FleetCluster{
		...
		Status: kaprov1alpha1.FleetClusterStatus{
			Conditions: []metav1.Condition{
				{Type: kaprov1alpha1.ConditionTypeStalled, Status: metav1.ConditionTrue, Reason: "BootstrapExpired"},
			},
		},
	}
	r, c := newBootstrapReconciler(t, fc)
	...
	if cond := apimeta.FindStatusCondition(got.Status.Conditions, kaprov1alpha1.ConditionTypeReconciling); cond == nil ||
		cond.Status != metav1.ConditionFalse || cond.Reason != "Suspended" {
		t.Errorf("Reconciling condition = %+v; want False/Suspended", cond)
	}
	if cond := apimeta.FindStatusCondition(got.Status.Conditions, kaprov1alpha1.ConditionTypeStalled); cond != nil {
...
func TestReconcile_NoBootstrapSpec_NoOp(t *testing.T) {
	fc := &kaprov1alpha1.FleetCluster{
		...
		Spec:       kaprov1alpha1.FleetClusterSpec{}, // no Bootstrap
	}
	r, _ := newBootstrapReconciler(t, fc)
...
func TestReconcile_NoBootstrapSpec_StaleFinalizer(t *testing.T) {
	fc := &kaprov1alpha1.FleetCluster{
		...
		Spec: kaprov1alpha1.FleetClusterSpec{}, // bootstrap removed
	}
	r, c := newBootstrapReconciler(t, fc)
	...
	if containsString(got.Finalizers, kaprov1alpha1.FleetClusterFinalizer) {
...
func TestReconcile_AddsFinalizer(t *testing.T) {
	fc := &kaprov1alpha1.FleetCluster{
		ObjectMeta: metav1.ObjectMeta{Name: "de-prod-01"},
		Spec: kaprov1alpha1.FleetClusterSpec{
			Bootstrap: &kaprov1alpha1.FleetClusterBootstrapSpec{TTL: "1h"},
		},
	}
	r, c := newBootstrapReconciler(t, fc)
...
// TestClassifyCSR_RenewalAccepted verifies that a CSR whose Username matches
// the issued cluster cert identity (kapro-cluster:<name>) AND whose Groups
// include kapro:cluster-controllers is recognised as a renewal — not as
// "unknown" (which would be silently ignored).
func TestClassifyCSR_RenewalAccepted(t *testing.T) {
	r, _ := newBootstrapReconciler(t)
	r.PodNamespace = "kapro-system"
	clusterName := "de-prod-01"

	csr := makeTestCSR(t, csrCNPrefix+clusterName, []string{csrOrganization},
		csrCNPrefix+clusterName) // Username == the cert CN (K8s x509 authenticator behavior)
	csr.Spec.Groups = []string{csrOrganization}

	fc := &kaprov1alpha1.FleetCluster{ObjectMeta: metav1.ObjectMeta{Name: clusterName}}
	if got := r.classifyCSR(csr, fc); got != csrKindRenewal {
		t.Errorf("classifyCSR for renewal CSR = %v, want csrKindRenewal", got)
	}
}

// TestClassifyCSR_RenewalRejectedWithoutGroup defends against a forged
// renewal where the Username matches but the Groups don't include
// kapro:cluster-controllers (a leaked cert presented by a different
// identity).
func TestClassifyCSR_RenewalRejectedWithoutGroup(t *testing.T) {
	r, _ := newBootstrapReconciler(t)
	clusterName := "de-prod-01"

	csr := makeTestCSR(t, csrCNPrefix+clusterName, []string{csrOrganization},
		csrCNPrefix+clusterName)
	csr.Spec.Groups = nil // no group — must be rejected

	fc := &kaprov1alpha1.FleetCluster{ObjectMeta: metav1.ObjectMeta{Name: clusterName}}
	if got := r.classifyCSR(csr, fc); got != csrKindUnknown {
		t.Errorf("classifyCSR without correct Group = %v, want csrKindUnknown", got)
	}
}

// TestHandleRenewalCSR_ApprovesWhenActive verifies the happy path: an active
// FleetCluster gets its renewal CSR approved without bootstrap-slot checks.
func TestHandleRenewalCSR_ApprovesWhenActive(t *testing.T) {
	clusterName := "de-prod-01"
	csr := makeTestCSR(t, csrCNPrefix+clusterName, []string{csrOrganization},
		csrCNPrefix+clusterName)
	csr.Spec.Groups = []string{csrOrganization}
	csr.Name = "csr-renewal-active"
	...
	fc := &kaprov1alpha1.FleetCluster{
		...
		Status: kaprov1alpha1.FleetClusterStatus{
			Bootstrap: &kaprov1alpha1.FleetClusterBootstrapStatus{
				Used:         true,
				BoundCSRName: "previously-approved-csr",
			},
		},
	}
	r, _ := newBootstrapReconciler(t, fc)
	...
	if err := r.handleRenewalCSR(context.Background(), fc, csr); err != nil {
		t.Fatalf("handleRenewalCSR: %v", err)
	}
	got, _ := fakeClient.CertificatesV1().CertificateSigningRequests().Get(context.Background(), csr.Name, metav1.GetOptions{})
	if !isCSRApproved(got) {
		t.Errorf("renewal CSR should be approved; status=%+v", got.Status)
	}
}

// TestHandleRenewalCSR_DeniedWhenSuspended verifies an operator can quarantine
// a misbehaving cluster by setting spec.suspend=true — the renewal flow MUST
// reject so the cluster's cert eventually expires.
func TestHandleRenewalCSR_DeniedWhenSuspended(t *testing.T) {
	clusterName := "de-prod-01"
	csr := makeTestCSR(t, csrCNPrefix+clusterName, []string{csrOrganization},
		csrCNPrefix+clusterName)
	csr.Spec.Groups = []string{csrOrganization}
	csr.Name = "csr-renewal-suspended"
	...
	got, _ := fakeClient.CertificatesV1().CertificateSigningRequests().Get(context.Background(), csr.Name, metav1.GetOptions{})
	if !isCSRDenied(got) {
		t.Errorf("suspended cluster's renewal CSR must be denied; status=%+v", got.Status)
	}
}
...
func TestMatchesFleetCluster_RejectsWrongSA(t *testing.T) {
	...
	fc := &kaprov1alpha1.FleetCluster{ObjectMeta: metav1.ObjectMeta{Name: "de-prod-01"}}
