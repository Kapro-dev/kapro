...
func TestProbeActuatorCapabilities(t *testing.T) {
	...
	result := Prober{DialOptions: bufDialOptions(listener)}.Probe(context.Background(), kaprov1alpha1.Plugin{
...
func TestProbeValidationFailures(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			spec: kaprov1alpha1.PluginSpec{Type: "other", Protocol: kaprov1alpha1.PluginProtocolGRPC, Endpoint: "dns:///plugin:9090"},
			want: "UnsupportedType",
		},
		{
			name: "unsupported protocol",
			spec: kaprov1alpha1.PluginSpec{Type: kaprov1alpha1.PluginTypeGate, Protocol: "http", Endpoint: "dns:///plugin:9090"},
			want: "UnsupportedProtocol",
		},
		{
			name: "missing endpoint",
			spec: kaprov1alpha1.PluginSpec{Type: kaprov1alpha1.PluginTypeGate, Protocol: kaprov1alpha1.PluginProtocolGRPC},
			want: "InvalidEndpoint",
		},
		{
			name: "invalid timeout",
			spec: kaprov1alpha1.PluginSpec{Type: kaprov1alpha1.PluginTypeGate, Protocol: kaprov1alpha1.PluginProtocolGRPC, Endpoint: "dns:///plugin:9090", Timeout: "soon"},
			want: "InvalidTimeout",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := Prober{}.Probe(context.Background(), kaprov1alpha1.Plugin{Spec: tt.spec})
			if result.Ready {
				t.Fatal("Ready = true")
			}
...
func TestValidateContractMessagesUsePluginContractName(t *testing.T) {
	tests := []struct {
		...
		{name: "actuator", pluginType: kaprov1alpha1.PluginTypeActuator, contract: "KAI"},
		{name: "gate", pluginType: kaprov1alpha1.PluginTypeGate, contract: "KGI"},
		{name: "planner", pluginType: kaprov1alpha1.PluginTypePlanner, contract: "KPI"},
	}
