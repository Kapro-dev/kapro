...
func NewActuatorAdapter(reg kaprov1alpha1.Plugin, client kaiv1alpha1.ActuatorServiceClient) (*ActuatorAdapter, error) {
...
func (a *ActuatorAdapter) IsConverged(ctx context.Context, cluster *kaprov1alpha1.Cluster, version, appKey string) (bool, error) {
	start := time.Now()
	result := "success"
...
func (a *ActuatorAdapter) Rollback(ctx context.Context, cluster *kaprov1alpha1.Cluster, previousVersion, appKey string) error {
	start := time.Now()
	result := "success"
...
func (a *ActuatorAdapter) IsAllConverged(ctx context.Context, cluster *kaprov1alpha1.Cluster, desiredVersions map[string]string) (bool, error) {
...
func clusterName(cluster *kaprov1alpha1.Cluster) (string, error) {
	if cluster == nil {
...
func actuatorCapabilitiesForPlugin(reg kaprov1alpha1.Plugin) actuator.Capabilities {
	...
	for _, capability := range reg.Status.Capabilities {
		...
		case kaiv1alpha1.HasCapability([]string{capability}, kaiv1alpha1.CapabilitySubstrateObjects):
			caps.SupportsSubstrateObjects = true
