...
func NewPlannerAdapter(reg kaprov1alpha1.Plugin, client kpiv1alpha1.PlannerServiceClient) (*PlannerAdapter, error) {
...
func (p *PlannerAdapter) PreFilter(ctx context.Context, state *planner.CycleState, req planner.Request, targets []kaprov1alpha1.Cluster) *planner.Status {
	start := time.Now()
	result := "success"
...
func (p *PlannerAdapter) Filter(_ context.Context, state *planner.CycleState, _ planner.Request, target kaprov1alpha1.Cluster) *planner.Status {
...
func (p *PlannerAdapter) Score(_ context.Context, state *planner.CycleState, _ planner.Request, target kaprov1alpha1.Cluster) (int64, *planner.Status) {
	planned, ok := p.plannedTarget(state, target.Name)
	if !ok {
		return 0, nil
	}
...
func promotionrunName(promotionrun *kaproruntimev1alpha1.PromotionRun) string {
	if promotionrun == nil {
		return ""
	}
...
func promotionrunVersion(promotionrun *kaproruntimev1alpha1.PromotionRun) string {
	if promotionrun == nil {
		return ""
	}
...
func plannerTargets(targets []kaprov1alpha1.Cluster) []*kpiv1alpha1.Target {
...
func targetReady(target kaprov1alpha1.Cluster) bool {
