...
func (r Registrar) RegisterReady(ctx context.Context, c client.Reader, actuatorReg *actuator.Registry, gateReg *gate.Registry, plannerFramework *planner.Framework) (int, error) {
	var list kaprov1alpha1.PluginList
...
func (r Registrar) RegisterOne(ctx context.Context, c client.Reader, reg kaprov1alpha1.Plugin, actuatorReg *actuator.Registry, gateReg *gate.Registry, plannerFramework *planner.Framework) error {
	if !isReadyForRuntime(reg) {
		return nil
	}
...
func (r Registrar) UnregisterOne(reg kaprov1alpha1.Plugin, actuatorReg *actuator.Registry, gateReg *gate.Registry, plannerFramework *planner.Framework) {
	switch reg.Spec.Type {
	case kaprov1alpha1.PluginTypeActuator:
		if actuatorReg != nil {
...
func (r Registrar) dial(ctx context.Context, c client.Reader, reg kaprov1alpha1.Plugin) (*grpc.ClientConn, error) {
	if err := validateRegistration(reg); err != nil {
		return nil, err
	}
...
func isReadyForRuntime(reg kaprov1alpha1.Plugin) bool {
	return reg.Status.Ready && reg.Status.ObservedGeneration == reg.Generation
}

func validateRegistration(reg kaprov1alpha1.Plugin) error {
...
func timeoutFor(reg kaprov1alpha1.Plugin) (time.Duration, error) {
	if reg.Spec.Timeout == "" {
